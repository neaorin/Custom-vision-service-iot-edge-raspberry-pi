...
#os.environ['MODEL_FOLDER'] = 'lego position model'
...
def initialize():
    ...
    if 'MODEL_FOLDER' in os.environ:
        ...
        print('Using model from folder: {0}'.format(model_folder))
    else:
        print('Model folder was not found, defaulting to running folder.')

    print('Loading model...',end=''),
    with tf.gfile.FastGFile('{0}/{1}'.format(model_folder, filename), 'rb') as f:
        graph_def.ParseFromString(f.read())
        tf.import_graph_def(graph_def, name='')
    print('Success!')
    print('Loading labels...', end='')
    with open('{0}/{1}'.format(model_folder, labels_filename), 'rt') as lf:
        global labels
